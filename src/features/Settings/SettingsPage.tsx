...
export const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  const jumpToSection = (id: SectionId) => {
    setActiveSection(id);
    setMobilePage(mobilePages.includes(id) ? (id as MobilePage) : null);
  ...
  const onInvalid = (formErrors: typeof errors) => {
    if (formErrors.email) {
      if (mobile) openEmailSheet();
      else jumpToSection('account');
    }
  };
  ...
  > = {
    ...
    theme: { label: t('themeLabel'), icon: MdDarkMode, options: themeOptions },
    timeFormat: {
  ...
  if (mobile) {
    return (
      ...
      >
        ...
        <fieldset disabled={!draft.ready} className="min-w-0">
          {mobilePage === 'privacy' ? (
            <SettingsPushPage
                  ...
                  <SheetRow
                    ...
                    onClick={() =>
                      window.location.assign(
                        `/api/auth/logout?locale=${currentLocale}`,
                      )
                    }
                  />
                </SettingsGroup>
